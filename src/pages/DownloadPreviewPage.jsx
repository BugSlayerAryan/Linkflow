...
// } from "../utils/downloadHistory.js";

// import {
//   canCacheDownloadBlob,
//   saveDownloadBlob,
// } from "../utils/downloadBlobStore.js";
...
//   return /googlevideo\.com|redirector\.googlevideo\.com/i.test(String(url || ""));
// }

// function isYoutubePlatform(videoInfo = {}) {
//   return String(videoInfo?.platform || "").toLowerCase().includes("youtube");
// }

// function getBrowserDirectMuxedVideo(videoInfo = {}, selectedMedia = null) {
//   const options = Array.isArray(videoInfo?.video) ? videoInfo.video : [];

//   if (
//     selectedMedia?.hasAudio === true &&
//     selectedMedia?.url &&
//     isGoogleVideoUrl(selectedMedia.url)
//   ) {
...
// function triggerBrowserDirectDownload(url, fileName = "video.mp4") {
//   /**
//    * Cross-origin `download` can be ignored by browsers, but this still gives
//    * the browser the same direct media URL that works from the video 3-dot menu.
//    */
//   const anchor = document.createElement("a");
//   anchor.href = url;
//   anchor.download = fileName;
...
//       const youtubeBrowserDirectVideo =
//         format === "video" && isYoutubePlatform(videoInfo)
...
//           const muxedYoutubeFallback =
//             format === "video" && isYoutubePlatform(videoInfo)
//               ? getBrowserDirectMuxedVideo(videoInfo, selectedMedia)
//               : null;

//           if (muxedYoutubeFallback?.url && muxedYoutubeFallback?.hasAudio === true) {
//             stopProcessingTimer();
//             removeActiveDownload(downloadId);
...
//             addRecentDownload({
//               ...downloadMeta,
//               url: muxedYoutubeFallback.url,
//               previewUrl: muxedYoutubeFallback.url,
//               quality: muxedYoutubeFallback.quality || downloadMeta.quality,
//               size: muxedYoutubeFallback.size || downloadMeta.size,
//               width: muxedYoutubeFallback.width || downloadMeta.width,
//               height: muxedYoutubeFallback.height || downloadMeta.height,
//               aspectRatio:
...
function isXPlatform(videoInfo = {}, fallbackUrl = "") {
  const platform = String(videoInfo?.platform || "").toLowerCase();
  const pageUrl = String(videoInfo?.webpage_url || fallbackUrl || "").toLowerCase();
...
function isM3u8Media(media = {}) {
  const mediaUrl = String(media?.url || "").toLowerCase();
  const ext = String(media?.ext || media?.extension || "").toLowerCase();

  return ext === "m3u8" || mediaUrl.includes(".m3u8");
}

function isPlayableBrowserVideo(media = {}) {
  const mediaUrl = String(media?.url || "").toLowerCase();
  const ext = String(media?.ext || media?.extension || "").toLowerCase();

  if (!media?.url) return false;
  if (isM3u8Media(media)) return false;

  return ext === "mp4" || mediaUrl.includes(".mp4") || mediaUrl.includes("video.twimg.com");
}

function getProxyMediaUrl(mediaUrl = "") {
  if (!mediaUrl) return "";

  return `${getApiBaseUrl()}/api/v1/proxy-media?url=${encodeURIComponent(
    mediaUrl
  )}`;
}
...
function DownloadPreviewPage() {
  ...
  const selectedPreview =
    format === "video"
      ? isXPlatform(videoInfo, url)
        ? getProxyMediaUrl(selectedMedia?.url) ||
          videoInfo?.previewUrl ||
          videoInfo?.rawPreviewUrl ||
          ""
        : selectedMedia?.url || videoInfo?.rawPreviewUrl || videoInfo?.previewUrl || ""
  ...
  useEffect(() => {
    ...
    const getBestDefaultVideo = (items = []) => {
      return (
        items.find((item) => isPlayableBrowserVideo(item) && item.hasAudio) ||
        items.find((item) => isPlayableBrowserVideo(item)) ||
        items.find((item) => item.ext === "mp4") ||
        items[0] ||
        null
      );
  ...
  const handleFormatChange = (nextFormat) => {
    ...
    if (nextOptions.length > 0) {
      const preferred =
        nextFormat === "video"
          ? nextOptions.find((item) => isPlayableBrowserVideo(item) && item.hasAudio) ||
            nextOptions.find((item) => isPlayableBrowserVideo(item)) ||
            nextOptions.find((item) => item.ext === "mp4") ||
            nextOptions[0]
          : nextOptions[0];
